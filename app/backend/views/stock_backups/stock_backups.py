...
from ...models import Crop, Equipment, Supplies, Livestock, Transaction
    ...
    try:
        ...
        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
            ...
            for row in rows:
                table_data.append(
                    [
                        Paragraph(str(item) if item is not None else "", cell_style)
                        for item in row
                    ]
                )
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="{stock_chosen}_Backup.zip"'
        )

        return response
    except Exception as e:
        logger.log(
            f"Error while downloading backup of {stock_chosen} by {request.user}: {e}"
        )
        messages.error(request, str(e))
