...
from app.backend.functions.paginationFunction import (amount_to_go,
                                                      paginationFunction)
...
class TestPaginationFunction:
    def test_amount_to_go(self):
        ...
        assert isinstance(
            amount_to_go, int
        ), f"Amount to go must be an integer, got {type(amount_to_go).__name__}."
    ...
    def test_pagination_no_items(self):
        ...
        items = []
        page_obj, backward, forward, page_num = paginationFunction(
            items, page_number=None, num_per_page=num_per_page
        )
    ...
    def test_pagination_default_first_page(self, sample_items):
        ...
        page_obj, backward, forward, page_num = paginationFunction(
            sample_items, page_number=None, num_per_page=num_per_page
        )
        total_possible_pages = page_obj.paginator.num_pages
        ...
        assert list(forward) == list(
            range(
                page_num + 1,
                clamp(page_num + amount_to_go + 1, 1, total_possible_pages + 1),
            )
        )
    ...
    def test_pagination_middle_page_windows(self, sample_items):
        ...
        page_number = 3
        page_obj, backward, forward, page_num = paginationFunction(
            sample_items, page_number=page_number, num_per_page=num_per_page
        )
        total_possible_pages = page_obj.paginator.num_pages

        assert page_num == page_obj.number
        # If page 3, backward list should be [1, 2]
        assert list(backward) == list(
            range(max(page_number - amount_to_go, 1), page_num)
        )
        # If page 3, forward list should be [4, 5]
        assert list(forward) == list(
            range(
                page_number + 1,
                min(page_number + amount_to_go + 1, total_possible_pages + 1),
            )
        )
    ...
    def test_pagination_negative_or_zero_page(self, sample_items):
        ...
        """
        # Testing 0
        _, _, _, page_num_zero = paginationFunction(
            sample_items, page_number=0, num_per_page=10
        )
        assert page_num_zero == 1

        # Testing negative number
        _, _, _, page_num_neg = paginationFunction(
            sample_items, page_number=-5, num_per_page=10
        )
    ...
    def test_pagination_out_of_bounds_high_page(self, sample_items):
        ...
        page_obj, backward, forward, page_num = paginationFunction(
            sample_items, page_number=99, num_per_page=num_per_page
        )
        total_possible_pages = page_obj.paginator.num_pages
        ...
        assert list(backward) == list(
            range(max(total_possible_pages - amount_to_go, 1), total_possible_pages)
        )
    ...
    def test_pagination_invalid_string_value(self, sample_items, mocker):
        ...
        page_obj, backward, forward, page_num = paginationFunction(
            sample_items, page_number="abc", num_per_page=10
        )
