...
class TestEditLivestock:
    class TestUnauthenticatedRequests:
        def test_edit_livestock_unauthenticated_redirect(self, valid_minimal_livestock, client):
            """
    ...
    class TestEditSuccess:
        def test_edit_livestock_with_changes(self, logged_in_client, valid_minimal_livestock, mock_logger):
            ...
            name_change_msg = editStockNameChange(old_name, valid_minimal_livestock.name)
            mock_logger.assert_called_once_with(f"User {user} edited livestock: {old_name}{name_change_msg} (ID: {valid_minimal_livestock.id}).")

        def test_edit_livestock_no_changes(self, logged_in_client, valid_full_livestock, mock_logger):
            client, user = logged_in_client
            ...
            assert str(valid_full_livestock.next_vaccination_date) == payload["next_vaccination_date"]
            assert valid_full_livestock.notes == payload["notes"]

            name_change_msg = editStockNameChange(old_name, valid_full_livestock.name)
            mock_logger.assert_called_once_with(f"User {user} edited livestock: {old_name}{name_change_msg} (ID: {valid_full_livestock.id}).")

        
        def test_edit_livestock_empty_inputs(self, logged_in_client, valid_full_livestock, mock_logger):
            client, user = logged_in_client
            ...
            assert valid_full_livestock.age == None
            assert valid_full_livestock.weight == None
            assert valid_full_livestock.health_status == DEFAULT_FILLER_TEXT
            assert valid_full_livestock.purchase_price == None
            assert valid_full_livestock.current_value == None
            assert valid_full_livestock.next_vaccination_date == None
    ...
    class TestEditErrors:
        ...
        def test_edit_livestock_validation_error_input_too_long(self, logged_in_client, valid_minimal_livestock, mock_logger):
            """
            ...
            assert Livestock.objects.count() == 1
            assert Livestock.objects.filter(name=payload["name"]).count() == 0
            assert Livestock.objects.filter(name=valid_minimal_livestock.name).count() == 1
            assert valid_minimal_livestock.name == old_name
            ...
            mock_logger.assert_called_once_with(f"Livestock edit error by {user}: Livestock name input must be less than or equal to {DEFAULT_TEXT_MAX_LENGTH} characters.")

        def test_edit_livestock_validation_error_input_wrong_type(self, logged_in_client, valid_minimal_livestock, mock_logger):
            """
            ...
            messages = list(get_messages(response.wsgi_request))
            assert len(messages) == 1
            assert "An unexpected error occurred while editing the livestock" in str(messages[0])

            mock_logger.assert_called()
            assert "Unexpected error during livestock edit" in mock_logger.call_args[0][0]