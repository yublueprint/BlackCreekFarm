...
class TestAddLivestock():
    ...
    class TestAddSuccess:
        def test_add_livestock_success(self, logged_in_client, mock_logger):
            ...
            assert str(livestock.next_vaccination_date) == payload["next_vaccination_date"]
            assert livestock.notes == payload["notes"]

            mock_logger.assert_called_once_with(f"User {user} added livestock: {livestock.name} (ID: {livestock.id}).")
        ...
        def test_add_livestock_empty_inputs(self, logged_in_client, mock_logger):
            ...
            assert livestock.age == None
            assert livestock.weight == None
            assert livestock.health_status == DEFAULT_FILLER_TEXT
            assert livestock.purchase_price == None
            assert livestock.current_value == None
            assert livestock.next_vaccination_date == None
            assert livestock.notes == ""

            mock_logger.assert_called_once_with(f"User {user} added livestock: {livestock.name} (ID: {livestock.id}).")

    class TestAddErrors:
        def test_add_livestock_validation_error_input_too_long(self, logged_in_client, mock_logger):
            """
            ...
            mock_logger.assert_called_once_with(f"Livestock creation error by {user}: Livestock name input must be less than or equal to {DEFAULT_TEXT_MAX_LENGTH} characters.")

        def test_add_livestock_validation_error_input_wrong_type(self, logged_in_client, mock_logger):
            """
            ...
            messages = list(get_messages(response.wsgi_request))
            assert len(messages) == 1
            assert "An unexpected error occurred while adding the livestock" in str(messages[0])

            mock_logger.assert_called()
            assert "Unexpected error during livestock creation" in mock_logger.call_args[0][0]