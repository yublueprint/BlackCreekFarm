...
class TestTransactionList:
    class TestDefaultStates:
        def test_transaction_list_unauthenticated_redirect(self, valid_full_transaction, client):
            """
            ...
            # Access supples list by giving in a VALID id in the url (e.g. transaction/id/45 if transaction with ID 45 exists).
            url = reverse("load_transaction", kwargs={"id": valid_full_transaction.id})
            response = client.get(url)
            assert response.status_code == 302
            assert "login" in response.url

            # Access supples list by giving in an INVALID id in the url (e.g. transaction/id/9999 if transaction with ID 9999 does not exist).
        ...
        def test_transaction_list_empty(self, logged_in_client, mock_logger):
            ...
            mock_logger.assert_called_once_with(f"User {user} viewed transaction list (page {1}).")

        def test_transaction_list_with_item(self, logged_in_client, valid_full_transaction, mock_logger):
            """
            Tests the machinery of the transaction list view. 
            Checks if the right variables are given.
        ...
        def test_transaction_list_error(self, logged_in_client, mocker, mock_logger):
            ...
            mock_search = mocker.patch("app.backend.views.transactions.transaction.search_filtering")
            ...
            assert f"Error in transactions view by {user}: {exception_message}" in mock_logger.call_args[0][0]

    class TestIDGivenURL:
        def test_transaction_list_id_success(self, logged_in_client, valid_full_transaction, mock_logger):
            """
            Tests if ID of an existing transaction is given in the url (e.g. transaction/id/45), it shows that one transaction with that ID.
            """
            client, user = logged_in_client
            ...
            assert len(response.context["page_obj"].object_list) == 1
            assert valid_full_transaction == response.context["page_obj"].object_list[0]
            assert f"ID: {valid_full_transaction.id}" in response.context["search_filters_applied"]
    ...
    class TestSearchFiltering:
        def test_transaction_list_search_filtering(self, logged_in_client, valid_full_transaction, mock_logger):
            """
            ...
            assert response.status_code == 200
            assert f"Item Name: {name_search}" in response.context["search_filters_applied"]
            assert len(response.context["search_filters_applied"]) == 1
