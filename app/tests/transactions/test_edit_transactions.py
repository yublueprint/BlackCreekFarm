...
class TestEditTransaction:
    class TestUnauthenticatedRequests:
        def test_edit_transaction_unauthenticated_redirect(self, valid_full_transaction, client):
            """
    ...
    class TestEditSuccess:
        def test_edit_transaction_with_changes(self, logged_in_client, valid_full_transaction, mock_logger):
            ...
            name_change_msg = editStockNameChange(old_name, valid_full_transaction.item_name)
            mock_logger.assert_called_once_with(f"User {user} edited transaction: {old_name}{name_change_msg} (ID: {valid_full_transaction.id}).")

        def test_edit_transaction_no_changes(self, logged_in_client, valid_full_transaction, mock_logger):
            client, user = logged_in_client
        ...
        def test_edit_transaction_empty_inputs(self, logged_in_client, valid_full_transaction, mock_logger):
            client, user = logged_in_client
    ...
    class TestEditErrors:
        ...
        def test_edit_transaction_validation_error_input_too_long(self, logged_in_client, valid_full_transaction, mock_logger):
            """
            ...
            valid_full_transaction.refresh_from_db()
            assert Transaction.objects.count() == 1
            assert Transaction.objects.filter(item_name=payload["item_name"]).count() == 0
            assert Transaction.objects.filter(item_name=valid_full_transaction.item_name).count() == 1
            ...
            mock_logger.assert_called_once_with(f"Transaction edit error by {user}: Transaction notes input must be less than or equal to {TEXTBOX_MAX_LENGTH} characters.")