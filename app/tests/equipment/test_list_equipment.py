...
class TestEquipmentList:
    class TestDefaultStates:
        def test_equipment_list_unauthenticated_redirect(
            ...
            # Access equipmentes list by giving in a VALID id in the url
            # (e.g. equipment/id/45 if equipment with ID 45 exists).
            url = reverse("load_equipment", kwargs={"id": valid_minimal_equipment.id})
            response = client.get(url)
            assert response.status_code == 302
            assert "login" in response.url

            # Access equipmentes list by giving in an INVALID id in the url
            # (e.g. equipment/id/9999 if equipment with ID 9999 does not exist).
        ...
        def test_equipment_list_empty(self, logged_in_client, mock_logger):
            ...
            mock_logger.assert_called_once_with(
                f"User {user} viewed equipment list (page {1})."
            )

        def test_equipment_list_with_item(
            ...
            assert (
                valid_minimal_equipment == response.context["page_obj"].object_list[0]
            )
            assert response.context["search_filters_applied"] == []
            assert response.context["max_textbox_length"] == TEXTBOX_MAX_LENGTH
            assert response.context["max_input_text_length"] == DEFAULT_TEXT_MAX_LENGTH
        ...
        def test_equipment_list_error(self, logged_in_client, mocker, mock_logger):
            ...
            mock_search = mocker.patch(
                "app.backend.views.equipment.equipment.search_filtering"
            )
            ...
            assert (
                f"Error in equipment view by {user}: {exception_message}"
                in mock_logger.call_args[0][0]
            )

    class TestIDGivenURL:
        def test_equipment_list_id_success(
            self, logged_in_client, valid_minimal_equipment, mock_logger
        ):
            """
            Tests if ID of an existing equipment is given in the url
            (e.g. equipment/id/45), it shows that one equipment with that ID.
            """
            client, user = logged_in_client
            ...
            assert (
                f"ID: {valid_minimal_equipment.id}"
    ...
    class TestSearchFiltering:
        def test_equipment_list_search_filtering(
            self, logged_in_client, valid_minimal_equipment, mock_logger
        ):
            """
            ...
            Equipment.objects.create(name=new_item_name, category="Farm Tool Category")
