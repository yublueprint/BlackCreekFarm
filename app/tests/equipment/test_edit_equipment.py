...
class TestEditEquipment:
    class TestUnauthenticatedRequests:
        def test_edit_equipment_unauthenticated_redirect(self, valid_minimal_equipment, client):
            """
    ...
    class TestEditSuccess:
        def test_edit_equipment_with_changes(self, logged_in_client, valid_minimal_equipment, mock_logger):
            ...
            name_change_msg = editStockNameChange(old_name, valid_minimal_equipment.name)
            mock_logger.assert_called_once_with(f"User {user} edited equipment: {old_name}{name_change_msg} (ID: {valid_minimal_equipment.id}).")

        def test_edit_equipment_no_changes(self, logged_in_client, valid_full_equipment, mock_logger):
            client, user = logged_in_client
            ...
            assert str(valid_full_equipment.maintenance_due) == payload["maintenance_due"]
            assert str(valid_full_equipment.next_checkup) == payload["next_checkup"]
            assert str(valid_full_equipment.warranty_expiry) == payload["warranty_expiry"]
            ...
            assert valid_full_equipment.service_interval_days == payload["service_interval_days"]
            assert valid_full_equipment.maintenance_history == payload["maintenance_history"]
            assert valid_full_equipment.notes == payload["notes"]

            name_change_msg = editStockNameChange(old_name, valid_full_equipment.name)
            mock_logger.assert_called_once_with(f"User {user} edited equipment: {old_name}{name_change_msg} (ID: {valid_full_equipment.id}).")

        
        def test_edit_equipment_empty_inputs(self, logged_in_client, valid_full_equipment, mock_logger):
            client, user = logged_in_client
            ...
            assert valid_full_equipment.purchase_date == None
            assert valid_full_equipment.maintenance_due == None
            assert valid_full_equipment.next_checkup == None
            assert valid_full_equipment.warranty_expiry == None
    ...
    class TestEditErrors:
        ...
        def test_edit_equipment_validation_error_input_too_long(self, logged_in_client, valid_minimal_equipment, mock_logger):
            """
            ...
            assert Equipment.objects.count() == 1
            assert Equipment.objects.filter(name=payload["name"]).count() == 0
            assert Equipment.objects.filter(name=valid_minimal_equipment.name).count() == 1
            assert valid_minimal_equipment.name == old_name
            ...
            mock_logger.assert_called_once_with(f"Equipment edit error by {user}: Equipment name input must be less than or equal to {DEFAULT_TEXT_MAX_LENGTH} characters.")

        def test_edit_equipment_validation_error_input_wrong_type(self, logged_in_client, valid_minimal_equipment, mock_logger):
            """
            ...
            messages = list(get_messages(response.wsgi_request))
            assert len(messages) == 1
            assert "An unexpected error occurred while editing the equipment" in str(messages[0])

            mock_logger.assert_called()
            assert "Unexpected error during equipment edit" in mock_logger.call_args[0][0]